...
func (w *ResticWrapper) RunRestore(restoreOptions RestoreOptions, targetRef kmapi.TypedObjectReference) (*RestoreOutput, error) {
	...
	restoreOutput := &RestoreOutput{
		RestoreTargetStatus: RestoreMemberStatus{
			Ref:   targetRef,
...
func (w *ResticWrapper) RunParallelRestore(restoreOptions []RestoreOptions, targetRef kmapi.TypedObjectReference, maxConcurrency int) (*RestoreOutput, error) {
...
func (w *ResticWrapper) Dump(dumpOptions DumpOptions, targetRef kmapi.TypedObjectReference) (*RestoreOutput, error) {
	// Start clock to measure total restore duration
	startTime := time.Now()
...
func (w *ResticWrapper) ParallelDump(dumpOptions []DumpOptions, targetRef kmapi.TypedObjectReference, maxConcurrency int) (*RestoreOutput, error) {
	// WaitGroup to wait until all go routine finish
	wg := sync.WaitGroup{}
	// concurrencyLimiter channel is used to limit maximum number simultaneous go routine
	concurrencyLimiter := make(chan bool, maxConcurrency)
